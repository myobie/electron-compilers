...
var PostCSSCompiler = function (_CompilerBase) {
  ...
  _createClass(PostCSSCompiler, [{
    ...
    value: function compileSync(sourceCode, filePath, compilerContext) {
      console.dir({ sync: true, sourceCode: sourceCode, filePath: filePath });

      if (!postcss) {
        var compiler = require('postcss');
        var plugins = this.plugins.map(function (name) {
          return require(name);
        });
        postcss = compiler.process(plugins);
      }
      ...
      var result = postcss.process(sourceCode, opts);
