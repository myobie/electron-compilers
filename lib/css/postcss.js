...
var PostCSSCompiler = function (_CompilerBase) {
  ...
  function PostCSSCompiler() {
    ...
    var possibleFile = _path2.default.join(process.cwd(), '.postcss.js');
    console.dir({ possibleFile: possibleFile });
    try {
      _fs2.default.statSync(possibleFile);
    } catch (e) {
      console.error(e);
      possibleFile = null;
    }
    if (possibleFile) {
      ...
      try {
        _this.processor = require(possibleFile);
      } catch (e) {
        console.error(e);
      }
    } else {
      _this.processor = function (postcss) {
        return postcss();
      };
    }
    console.log(_this.processor);
  ...
  _createClass(PostCSSCompiler, [{
    ...
    value: function () {
      var _ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee(sourceCode, filePath, compilerContext) {
        var compiler, thisPath, paths, opts, result;
